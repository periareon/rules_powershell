...
def _pwsh_entrypoint_impl(ctx):
    pwsh_toolchain = ctx.attr._pwsh_toolchain_exec[platform_common.ToolchainInfo]
    ...
    inputs.append(substitutions_file)
    ctx.actions.write(
    ...
    args.add("-SubstitutionsFile", substitutions_file)

    ctx.actions.run(
        mnemonic = "PwshEntrypointTemplate",
        executable = pwsh_toolchain.pwsh,
        arguments = [args],
        inputs = inputs,
        tools = depset([ctx.file._maker], transitive = [pwsh_toolchain.all_files]),
        outputs = [output],
    )
...
pwsh_entrypoint = rule(
    doc = "Generates the entrypoint for `pwsh_*` executable rules.",
    implementation = _pwsh_entrypoint_impl,
    attrs = {
        ...
        "_maker": attr.label(
            ...
            default = Label("//powershell/private:entrypoint_maker.ps1"),
        ),
        "_pwsh_toolchain_exec": attr.label(
            cfg = "exec",
            default = Label("//powershell:current_pwsh_toolchain"),
